...
def light_atk(k):
    back.press_sec(k, keys=['h'], sec=0.2, delay=1.0)
    
def heavy_atk(k):
    back.press_sec(k, keys=['u'], sec=0.2, delay=1.8)
    
def f_roll(k, sec=0.2):
    back.press_sec(k, keys=['w',' '], sec=1.3)

def b_roll(k, sec=0.2):
    back.press_sec(k, keys=['s',' '], sec=1.3)
    
def l_roll(k, sec=0.2):
    back.press_sec(k, keys=['a',' '], sec=1.3)
    
def r_roll(k, sec=0.2):
    back.press_sec(k, keys=['d',' '], sec=1.3)

def drink_estus(k, sec=0.2):
    back.press_sec(k, keys=['e'], sec=2.5)
